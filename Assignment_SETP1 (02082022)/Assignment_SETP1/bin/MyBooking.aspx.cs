...
namespace Assignment_SETP1
{
    public partial class MyBooking : Page
    {
        ...
        protected void GVbind(List<BookingInfo> bookings)
        {
            ...
            {
                con.Open();
                SqlCommand cmd = new SqlCommand("SELECT ROW_NUMBER() OVER (ORDER BY (SELECT NULL)) AS SN, B.bookingCreationDateTime, B.BookingID, B.customerComment, B.appoointmentDateTime,  P.customerName, P.customerAddress FROM Customer.CustomerBookingInfo B, Customer.CustomerProfileInfo P WHERE B.customerUserID = P.userID AND appoointmentStatus = 'NULL'; ", con);
        ...
        protected void ViewButton1_Click(object sender, CommandEventArgs e)
        {
            ...
            Response.Redirect("ViewBooking.aspx");
                //Response.Redirect("ViewBooking.aspx?BookingRef=" + selectedBookingRef);
